...
export default function EcrituresTab({ dossierId }: { dossierId: string }) {
  ...
  async function load() {
    setLoading(true)
    const [{ data: categoriesData }, { data: piecesData }, { data: ecrituresData }] = await Promise.all([
  ...
  const piecesEligibles = pieces.filter((p) => p.montant_ttc != null && !!categorieById(p.categorie_id)?.compte_comptable)
  ...
  return (
    <>
      <div className="brouillon-banner">
        <strong>Brouillon</strong> — ces écritures sont une proposition à partir des pièces validées, pas une
        comptabilité tenue. Elles doivent être vérifiées et validées par un expert-comptable inscrit avant tout
        usage fiscal ou légal.
      </div>
